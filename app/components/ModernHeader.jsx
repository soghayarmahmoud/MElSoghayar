        ...
        <div className="flex items-center justify-between rounded-full border border-white/10 bg-[#121212]/75 px-4 py-2.5 shadow-[0_10px_30px_rgba(0,0,0,0.18)] backdrop-blur-xl">
          <div className="flex items-center gap-2">
            <Link href="#hero" className="text-2xl md:text-3xl font-bold tracking-tight transition-transform hover:scale-105">
              <span className="text-amber-400">Elsogh</span>
          ...
          <div className="hidden items-center space-x-1 md:flex">
            {navItems.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="rounded-full px-4 py-2 text-sm font-medium text-[#d4d4d4] transition-colors hover:text-amber-300"
              >
                {item.name}
              </Link>
            ))}
          ...
          <div className="flex items-center space-x-3">
            <button
              ...
              className="flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-[#1a1a1a] text-[#f5f5f5] transition-colors hover:bg-amber-400 hover:text-[#121212] md:hidden"
            >
          ...
          <div className="mt-3 rounded-[22px] border border-white/10 bg-[#121212]/90 p-3 shadow-[0_18px_40px_rgba(0,0,0,0.25)] md:hidden">
            {navItems.map((item) => (
              <Link
                ...
                className="block rounded-full px-4 py-3 text-sm font-medium text-[#d4d4d4] transition-colors hover:bg-white/5 hover:text-amber-300"
              >
                {item.name}
              </Link>
            ))}
